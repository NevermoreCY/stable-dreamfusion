...
class StableDiffusion(nn.Module):
    ...
    def train_step(self, text_embeddings, pred_rgb, pred_depth, pred_image , azimuth,guidance_scale=100, as_latent=False, grad_scale=1):
        ...
        with torch.no_grad():
            ...
            noise_control = noise.clone()
            # print("random noise shape :" , noise.shape)
            latents_noisy = self.scheduler.add_noise(latents, noise, t)
            # print("latents_noisy shape", latents_noisy.shape)
            # pred noise
            latent_model_input = torch.cat([latents_noisy] * 2)
            # print("latents_noisy shape after concatetion ", latent_model_input.shape , torch.max(latent_model_input), torch.min(latent_model_input))
            ...
            # for control net
            if self.control:
                control_latent_model_input =  torch.cat([noise_control] * 2)
                control_latent_model_input = self.scheduler.scale_model_input(control_latent_model_input, t)
        ...
        if self.control:
            # guidance for control net

            print("*********using control")
            control_noise_pred_uncond, control_noise_pred_text = control_noise_pred.chunk(2)
            control_noise_pred = control_noise_pred_uncond + guidance_scale * (
